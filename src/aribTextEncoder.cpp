...
// ARIB STD-B62 resolution/audio-format pictograms (4K/8K/22.2ch/...).
// These overlap with information TVTest (and similarly capable players)
// can show natively as icons, so converting them to bracketed text is
// optional and controlled by [epg] convertResolutionGaiji in the ini.
constexpr TextReplacement ResolutionGaijiTable[] = {
    {u8"\U0001F19B", u8"[3D]"},
...
constexpr TextReplacement GaijiTable[] = {
    // HDR has no equivalent structured descriptor that players can derive
    // it from independently (unlike resolution/channel count), so always
    // keep it as readable text instead of gating it behind
    // convertResolutionGaiji.
    {u8"\U0001F1A7", u8"[HDR]"},
    {u8"\U0001F23B", u8"[配]"},
    {u8"\U000032FF", u8"令和"},
