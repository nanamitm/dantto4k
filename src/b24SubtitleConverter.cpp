...
bool B24SubtitleConverter::convert(const std::string& input, std::list<B24SubtitleOutput>& output, B24::PESData::PESType pesType,
    MmtTlv::SubtitleResolution resolution, const arib::ttml::DrcsGlyphMap& drcsGlyphs) {
    ...
    auto convert_result = arib::ttml::convert_to_b24(*resolve_result.document, sync_mode, resolution, drcsGlyphs);
    ...
    for (auto& converted : convert_result.outputs) {
        ...
        // The DRCS patterns must be defined before the statement body that
        // designates them.
        if (converted.drcs.size()) {
            captionStatementData.dataUnits.push_back({ std::move(converted.drcs), B24::DataUnitParameter::DRCS1Byte });
        }
